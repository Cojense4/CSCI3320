...
class SinglyLinkedList:  # Single Linked List
    ...
    def isempty(self):  # Boolean to check if the list is empty
        if self.head is None:
            return True
        else:
            return False

    def prepend(self, data):  # Encapsulate the data in a Node
        node = Node(data)
        if self.head is None:
            self.head = node
            self.tail = node
    ...
    def append(self, data):  # Encapsulate the data in a Node
        node = Node(data)
        if self.head is None:
            self.head = node
            self.tail = node
...
def printInOrder(list):  # Recurse starting with the head node
...
def reverseList(list):  # Edit this function
    ...
    def reverseRecursion(node):
        if node is not head:
            list.append(list.delete_first_node())
